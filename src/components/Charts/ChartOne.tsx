...
const ChartOne: React.FC<LocationDataProps> = ({ filteredLocations }) => {
  ...
  const fetchDataFromAPI = async () => {
    try {
      const response = await fetch("https://random-number-generator-7jp6.onrender.com/value");
      const data = await response.json();

      const updatedSeries = series.map(s => ({
        ...s,
        data: s.name === "SLIIT" ? [...s.data.slice(1), data] : s.data, // Only update SLIIT data
      }));

      setSeries(updatedSeries); // Update the series with the new data
    } catch (error) {
  ...
  useEffect(() => {
    const initialSeries = filteredLocations.map(location => ({
      name: location.city,
      data: Array(8).fill(location.city === "Malabe" ? 80 : location.city === "Kaduwela" ? 60 : location.city === "SLIIT" ? 30 : 10), // Set constant values for Malabe and Kaduwela
    }));
  ...
  return (
    <div className="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-8">
      <h1 className="text-[16pt] font-semibold text-center mb-[17px]">Real Time Garbage Levels</h1>
      <div className="flex flex-wrap items-start justify-between sm:flex-nowrap mx-[14px]">
      ...
      <div className="mt-2">
        ...
        <button onClick={downloadJPG} className="rounded-md bg-green-500 px-4 py-2 text-white">
          Download JPG
        </button>
      </div>
    </div>
